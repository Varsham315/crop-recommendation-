...
def speak():
    ...
    return jsonify({"message": "Audio generated!", "audio_url": "/static/speech.mp3"})
...
def predict():
    if request.method == "POST":
        ...
        print("Processed Input Features:", input_features)
        inf = xgb.predict([input_features])
        inf = inf[0]
        print(inf)
        value = mapper[inf]
